...
- **When triggered**: `intent.md` is approved and work is high-stakes or complex.
- **Action**: Compress structure and requirements into `projects/<slug>/spec.md` in one sitting.
- **Rules**:
  - Lock section headings, reading level, citation standards, and explicit non-goals.
  - Do NOT begin writing prose until `spec.md` is agreed upon.

### 3. Stage 3: Build → `draft.md`
- **When triggered**: `spec.md` and `context.md` are established.
...
### 4. Stage 4: Test → `review.md`
- **When triggered**: `draft.md` is completed.
- **Action**: Produce `projects/<slug>/review.md` following the two-layer review rubric:
...
### 5. Stage 5: Deploy → `final.md`
- **When triggered**: All Layer 2 flagged items in `review.md` are resolved and signed off.
- **Action**: Promote verified text to `projects/<slug>/final.md` with version frontmatter and author attribution.

### 6. Stage 6: Maintain → `feedback.md`
- **When triggered**: The deliverable receives real-world reception, comments, or critique.
- **Action**: Record feedback in `projects/<slug>/feedback.md` and define the follow-up intent prompt that restarts the loop for the next deliverable.

---

## 🛠️ Project Initialization Command Checklist

When the user asks to start a project:
1. Confirm or suggest the project slug (e.g. `policy-brief-gpu-allocation`).
2. Scaffold `projects/<project-slug>/` with `context.md`, `intent.md`, and initial files from `projects/_template-project/`.
